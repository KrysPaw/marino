...
export class Commander {
  ...
  private subscriptions: { [A in LocalActionType]?: Record<Subscription<A>['id'], Subscription<A>> } = {};

  private constructor() {
    if (Commander.instance) {
      throw new Error('Commander is a singleton and has already been instantiated.');
    }
  ...
  private getSubscriptionsListForAction<T extends LocalActionType>(action: T): Subscription<T>[] {
